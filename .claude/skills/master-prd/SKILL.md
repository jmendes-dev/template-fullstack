---
name: master-prd
description: Cria um PRD (Product Requirements Document) através de entrevista guiada com o usuário, salvo em ./plans/ no formato consumível pelo /master-plan. Usar quando o usuário disser "criar PRD", "novo produto", "documentar feature", "novo projeto", "começar projeto novo" ou pedir um documento de requisitos.
user-invocable: true
allowed-tools: Read, Write, Glob, Grep
---

Esta skill é invocada quando o usuário quer criar um PRD (Product Requirements Document). Siga os passos abaixo **em ordem**. O público-alvo são pessoas com pouca bagagem técnica — conduza a conversa de forma acolhedora, prática e sem jargões desnecessários.
...
5. **Detectar projeto novo**: marcar `projeto_novo = true` se **qualquer** uma destas condições for verdadeira: (a) `package.json` da raiz não existe, (b) `apps/api/package.json` não existe, (c) `packages/shared/package.json` não existe. Pastas vazias (`apps/`, `packages/shared/`) **não contam** como projeto existente — sem `package.json` real, é projeto novo. Se `projeto_novo = true`, vai perguntar deploy target na Rodada 2. Caso contrário, herdar o target do README/CLAUDE.md do projeto existente e **não perguntar**
...
- **Deploy target (só se `projeto_novo = true` do Passo 0)**: perguntar obrigatoriamente "Railway (PaaS) ou Portainer (on-prem)?" com uma frase de tradeoff cada. Registrar na seção "Decisões Arquiteturais" do PRD. Se o projeto não é novo, **não perguntar** — herdar do projeto existente
...
> Este contrato é **especificativo** — define o design, não descreve o código ainda existente. Após implementação, validar cada endpoint com `curl` e atualizar se divergir. Ver regra 18 do CLAUDE.md ("nunca inventar campos — verificar responses reais").
...
Pode incluir:
- **Deploy target**: Railway ou Portainer — **obrigatório apenas se PRD para projeto novo** (sem `apps/`/`packages/shared/`); se projeto existente, omitir (herdado)
