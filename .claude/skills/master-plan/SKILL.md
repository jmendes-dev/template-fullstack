...
Esta skill é invocada quando o usuário quer transformar um PRD (Product Requirements Document) em um **plano de implementação faseado**. Cada fase é uma fatia vertical — um pedaço fino que passa por todas as camadas ao mesmo tempo (banco, API, frontend, testes), entregando algo demonstrável.
...
6. Confira `docs/version-matrix.md` e `docs/tech-stack.md` para confirmar as versões das libs que o plano assumirá (Drizzle stable vs beta, React Router v7, Zustand v5, Zod v4, drizzle-zod ≥0.8.3, etc.) — o plano deve referenciar as mesmas versões pinadas no template
...
>
> O master-plan funciona a partir de um PRD pronto — é o documento que descreve o que você quer construir, pra quem, e como.
>
> Se você ainda não tem um, posso te ajudar a criar com o `/master-prd`. Quer fazer isso primeiro?
>
...
**Versões**: fixar versões mínimas da stack conforme `docs/version-matrix.md` — especialmente Hono ≥4.12.14 (CVE-2026-29045 + CVE-2026-39407), Bun ≥1.3, Zod v4.0+, TypeScript ≥6.0.
